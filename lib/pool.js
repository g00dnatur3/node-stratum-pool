   ...
   let retryCount = 30
    ...
    function SetupPeer() {
        ...
        _this.peer.on('connected', function () {
            ...
            emitErrorLog(`p2p connection failed - likely incorrect host or port: ${JSON.stringify(options.p2p)}`);
        }).on('socketError', function (e) {
            emitErrorLog('p2p had a socket error ' + JSON.stringify(e));
        }).on('error', function (msg) {
            emitWarningLog('p2p had an error ' + msg);
    ...
    function GetBlockTemplate(callback) {
        ...
        function getBlockTemplate(subsidy, retryCount=20) {
            var callConfig = {
                "capabilities": [
                    "coinbasetxn",
                    "workid",
            ...
            function (result) {
                    if (result.error) {
                     setTimeout(() => {
                         retryCount--
                         if (retryCount) getBlockTemplate(subsidy, retryCount)
                         else {
